...
// Faol o'yinlar RAM'da saqlanadi (tez-tez o'zgaradigan vaqtinchalik holat).
// Statistika (pul, g'alaba) esa MongoDB'da - shuning uchun restart bo'lsa ham yo'qolmaydi.
const games = new Map();       // chatId(string) -> game object
const activeChat = new Map();  // userId(string) -> chatId(string)
...
async function tickPhase(bot, chatId, expectedPhase) {
  const g = games.get(String(chatId));
  if (!g || g.status !== expectedPhase) return; // holat allaqachon o'zgargan bo'lishi mumkin
...
async function handleNightAction(bot, ctx, data) {
  ...
  const parts = data.split('_'); // mn_kill_123456
...
async function handleVote(bot, ctx, data) {
  ...
  const targetId = data.substring(3); // "mv_" prefiksidan keyingisi
